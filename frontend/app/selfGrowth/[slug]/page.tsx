...
export async function generateMetadata({
  ...
  return {
    title: post?.title ?? "Ինքնազարգացում",
    description: post?.content?.slice(0, 160) ?? "Ինքնազարգացում",
  };
}

export default async function SelfGrowthBySlug({
  params,
}: {
  params: { slug: string };
}) {
  const post = await getPost(params.slug);
  if (!post || post.category !== "self-growth") return notFound();
